...
class search {
	...
	function __construct($options=array()) {
		...
		foreach($pages as $page) {
			...
			if($matchedTotal) {
				$result[$page->uid()] = new Obj( array(
					'page' => $page,
					'searchHits' => $matchedTotal,
					'searchScore' => $score )
				);
			}
		...
		$pages = new Collection($result);
		$pages = $pages->sortBy('searchScore','desc');

		// replace array of Obj with array of pages
		$pages = new Collection($pages->pluck('page'));

		// add pagination
		if($this->paginate) $pages = $pages->paginate($this->paginate, array('method' => 'query'));
